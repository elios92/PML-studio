...
func TestPatchRuntimeBattleAnimationDirection(t *testing.T) {
	input := []byte(`                redraw=lambda: self._draw(
                    self._alive(self.state.get("party", [])) or attacker,
                    self._alive(self.enemy_party) or defender,
                    getattr(self, "_last_message", ""),
                ),
            )`)
	gotBytes, err := patchRuntimeBattleAnimationDirection(input)
	if err != nil {
		t.Fatalf("patch battle animation direction failed: %v", err)
	}
	got := string(gotBytes)
	if !strings.Contains(got, `opponent=attacker not in self.state.get("party", [])`) {
		t.Fatalf("opponent animation direction missing: %s", got)
	}
}

func TestRuntimeBattleAnimationPatchUsesConvertedDatabase(t *testing.T) {
	data, err := readEmbeddedRuntimeFile("game/battle_animation_player.py")
	if err != nil {
		t.Fatal(err)
	}
	gotBytes, err := patchRuntimeBattleAnimationCatalog(data)
	if err != nil {
		t.Fatalf("patch battle animation catalog failed: %v", err)
	}
	got := string(gotBytes)
	for _, want := range []string{
		...
		`self.catalog.animation_json(move_id, opponent=opponent)`,
		`return timing.get(str(key).lstrip("@"), default)`,
		`opponent: bool = False`,
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("converted Essentials battle animation support missing %q", want)
		}
	}
}
